# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

gradLogLikCppNoMeans <- function(theta, xptr) {
    .Call(`_cavaan_gradLogLikCppNoMeans`, theta, xptr)
}

gradLogLikCppOVMeans <- function(theta, xptr) {
    .Call(`_cavaan_gradLogLikCppOVMeans`, theta, xptr)
}

gradLogLikCppLVMeans <- function(theta, xptr) {
    .Call(`_cavaan_gradLogLikCppLVMeans`, theta, xptr)
}

gradLogLikNumericCpp <- function(theta, xptr, h = 1e-6) {
    .Call(`_cavaan_gradLogLikNumericCpp`, theta, xptr, h)
}

ViewModelCreation <- function(RModel, theta) {
    .Call(`_cavaan_ViewModelCreation`, RModel, theta)
}

createRcppModel <- function(RModel) {
    .Call(`_cavaan_createRcppModel`, RModel)
}

fillRcppModel <- function(xptr, theta) {
    .Call(`_cavaan_fillRcppModel`, xptr, theta)
}

logLikCpp <- function(theta, xptr) {
    .Call(`_cavaan_logLikCpp`, theta, xptr)
}

debugCppModel <- function(xptr, theta) {
    invisible(.Call(`_cavaan_debugCppModel`, xptr, theta))
}

getVariablesEquation <- function(expr) {
    .Call(`_cavaan_getVariablesEquation`, expr)
}

